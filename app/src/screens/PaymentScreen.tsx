...
export default function PaymentScreen() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      await paymentsApi.confirm(payRes.data.id, method === 'UPI' ? undefined : undefined);
      setSuccess(true);
      await refreshTables();
      setTimeout(() => { setActiveOrder(null); navigate('FloorPlan'); }, 2000);
    } catch (err: unknown) {
      ...
      <div style={{ width: 300, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {method === 'UPI' && (
          <div className="card" style={{ textAlign: 'center', padding: 22 }}>
            <div style={{
              width: 120, height: 120, background: 'var(--surface-2)', margin: '0 auto 10px',
              borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center',
              border: '1px solid var(--border)', color: 'var(--text-3)', fontSize: 12,
            }}>QR Placeholder</div>
            <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Scan to pay ₹{total.toFixed(2)}</div>
          </div>
        )}

...
export default function MenuScreen() {
  const { cart, addToCart, updateQty, cartTotal, cartCount, setScreen, setOrder, clearCart, logout, tableId, tableNumber, sessionActive } = useCustomer();
  ...
  if (loading) return (
    <div style={styles.loadingPage}>
      ...
      <p style={{ color: 'var(--text-2)', marginTop: 16 }}>Loading menu…</p>
    </div>
  );
  ...
  if (!sessionActive) return (
    <div style={styles.loadingPage}>
      ...
      <h2 style={{ fontFamily: "var(--font-ui)", color: 'var(--text)', marginTop: 16 }}>Restaurant is closed</h2>
      <p style={{ color: 'var(--text-2)' }}>No active session right now. Please try again later.</p>
    </div>
  );
      ...
      {modItem && (
        <div style={styles.overlay} onClick={() => setModItem(null)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ color: 'var(--text-2)', fontSize: 13, margin: '0 0 16px' }}>Choose add-ons (optional)</p>
            {modItem.modifiers.map((m) => (
              <label key={m.id} style={styles.modRow}>
                ...
                <span style={{ color: 'var(--accent)', fontWeight: 600, fontSize: 14 }}>+₹{m.price}</span>
              </label>
            ))}
            ...
            <div style={styles.cartItems}>
              {cart.map((item) => (
                <div key={item.key} style={styles.cartItemRow}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{item.productName}</div>
                    {item.modifiers.length > 0 && (
...
const styles: Record<string, React.CSSProperties> = {
  ...
  cardName: { fontWeight: 600, fontSize: 14, color: 'var(--text)', lineHeight: 1.3 },
  cardDesc: { fontSize: 12, color: 'var(--text-3)', margin: '4px 0 0', lineHeight: 1.4 },
  modTag: { fontSize: 11, color: 'var(--accent)', fontWeight: 600, margin: '4px 0 0' },
  cardFooter: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 },
  price: { fontWeight: 700, color: 'var(--text)', fontSize: 15 },
  addBtn: {
    background: 'var(--accent)',
    color: '#fff',
    border: 'none',
  ...
  qtyNum: { minWidth: 20, textAlign: 'center', fontWeight: 700, fontSize: 14, color: 'var(--accent)' },
  cartBar: {
    position: 'fixed',
    bottom: 24,
    left: '50%',
